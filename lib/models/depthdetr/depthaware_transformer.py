...
class DepthAwareTransformer(nn.Module):
    def __init__(
        ...
        self.decoder = DepthAwareDecoder(decoder_layer, num_decoder_layers, return_intermediate_dec, d_model,
                                         use_dab=use_dab, two_stage_dino=two_stage_dino)

        self.level_embed = nn.Parameter(torch.Tensor(num_feature_levels, d_model))
        print('self.level_embed', self.level_embed.shape)
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, depth_pos_embed=None, depth_pos_embed_ip=None,
                attn_mask=None):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            ...
            spatial_shape = (h, w)  # (48, 160)
            spatial_shapes.append(spatial_shape)

            src = src.flatten(2).transpose(1, 2)  # [B, 256, 48, 160] -> [B, 256, 7680] -> [B, 7680, 256]

            pos_embed = pos_embed.flatten(2).transpose(1, 2)  # [B, 256, 48, 160] -> [B, 256, 7680] -> [B, 7680, 256]
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)  # [B, 7680, 256]

            mask = mask.flatten(1)  # [B, 48, 160] -> [B, 7680]

            lvl_pos_embed_flatten.append(lvl_pos_embed)
            src_flatten.append(src)
            mask_flatten.append(mask)

        # [[B, 7680, 256], [B, 1920, 256], [B, 480, 256], [B, 120, 256]] -> [B, 10200, 256]
        src_flatten = torch.cat(src_flatten, 1)
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)  # [B, 10200, 256]
        mask_flatten = torch.cat(mask_flatten, 1)  # [B, 10200]
        spatial_shapes = torch.as_tensor(spatial_shapes, dtype=torch.long, device=srcs[0].device)  # [4, 2]
        # [7680, 1920,  480,  120] -> [7680,  9600, 10080, 10200] -> [7680,  9600, 10080] -> [0, 7680,  9600, 10080]
        ...
        if self.two_stage:
            ...
            refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1,
                                                   topk_proposals.unsqueeze(-1).repeat(1, 1, 6))  # unsigmoid
            refpoint_embed_ = refpoint_embed_undetach.detach()
            init_box_proposal = torch.gather(output_proposals, 1,
                                             topk_proposals.unsqueeze(-1).repeat(1, 1, 6)).sigmoid()  # sigmoid
            if self.training:
...
class DepthAwareDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False, d_model=None, use_dab=False,
                 two_stage_dino=False):
        super().__init__()
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                ...
                depth_pos_embed_ip=None, pos_embeds=None, attn_mask=None):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output,
                           ...
                           is_first=(lid == 0), depth_pos_embed_ip=depth_pos_embed_ip, pos_embeds=pos_embeds,
                           self_attn_mask=attn_mask, query_pos_un=query_pos_un)
