...
class DepthDETR(nn.Module):
    ...
    def __init__(self, backbone, depthaware_transformer, depth_predictor,
        ...
        self.two_stage_dino = two_stage_dino  # False
        ...
        if not two_stage:
            ...
            if not use_dab:
                self.query_embed = nn.Embedding(num_queries * group_num, hidden_dim * 2)  # 550, 512
            else:
        ...
        if num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),  # hidden_dim = 256
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        for l, feat in enumerate(features):
            ...
            srcs.append(self.input_proj[l](src))  # 用1x1 conv 改变特征维度到 [B, 256, h, w]
            masks.append(mask)
        ...
        if self.num_feature_levels > len(srcs):  # 4 > 3
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.input_proj[l](features[-1].tensors)  # [B, 256, 6, 20]
                else:
                    src = self.input_proj[l](srcs[-1])

                m = torch.zeros(src.shape[0], src.shape[2], src.shape[3]).to(torch.bool).to(src.device)  # [B, 6, 20]
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]  # [B, 6, 20]
                pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)  # [B, 256, 6, 20]
                srcs.append(src)
                masks.append(mask)
                pos.append(pos_l)
        ...
        if self.two_stage:   # False
            ...
            if self.training:
                ...
                query_embeds = torch.cat((tgt_embed, refanchor), dim=1)
        elif self.two_stage_dino:  # False
            query_embeds = None
        else:
            if self.training:
                query_embeds = self.query_embed.weight  # [550, 512]
            else:
                # only use one group in inference
                query_embeds = self.query_embed.weight[:self.num_queries]  # [50, 512]

        pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_pos_embed_ip = self.depth_predictor(srcs,
                                                                                                          masks[1],
                                                                                                          pos[1])
        """
        pred_depth_map_logits: [B, 81, 24, 80] 深度分类结果
        depth_pos_embed: [B, 256, 24, 80] 加入深度编码的经过encoder后的特征
        weighted_depth: [B, 24, 80] 预测深度
        depth_pos_embed_ip: [B, 256, 24, 80] 深度embedding
        """

        hs, init_reference, inter_references, inter_references_dim, enc_outputs_class, enc_outputs_coord_unact = \
            self.depthaware_transformer(srcs, masks, pos, query_embeds, depth_pos_embed, depth_pos_embed_ip)  # , attn_mask)
...
class SetCriterion(nn.Module):
    """ This class computes the loss for Depth-DETR.
...
def build(cfg):
    ...
    model = DepthDETR(
        ...
        num_queries=cfg['num_queries'],  # 50
        aux_loss=cfg['aux_loss'],  # True
        num_feature_levels=cfg['num_feature_levels'],  # 4
        with_box_refine=cfg['with_box_refine'],  # True
        two_stage=cfg['two_stage'],  # False
        init_box=cfg['init_box'],  # False
        use_dab=cfg['use_dab'],  # False
        two_stage_dino=cfg['two_stage_dino'])  # False
